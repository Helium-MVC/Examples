...
class apiController extends He2Controller {
	...
	/**
	 * The model to be called in the CRUD operation in the extened class
	 */
	protected $_model = null;

	/**
	 * The fields to call in a sql query
	 */
	protected $_fields = '*';

	/**
	 * The primary id, or can be an array if multiple primary
	 */
	protected $_id = null;

	/**
	 * Setups up parent controllers and opens up CORS
	 * 
	 * @param string $reg The registry
	...
	protected function create($data) {
		...
		if ($model->create($data, array('validate_options' => array(
				'display' => false,
				'event' => 'create'
			)))) {
	...
	public function update($data) {
		...
		if ($model) {
			...
			$model->update($data, array('validate_options' => array(
					'display' => false,
					'event' => 'update'
				)));
	...
	public function validate() {
		...
		if ($result) {
			$this->_jsonResponse(array(
				'status' => 0,
				'message' => $model->getValidationErrors()
			));
		} else {
			$this->_jsonResponse(array(
				'status' => 1,
	...
	/**
	 * For multiple dimensional array, ensures all the levels are utf-8 encoded
	 * 
	 * @param $input Either a string, array or object to be encoded
	 * 
	 * 
	 */
