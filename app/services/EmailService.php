...
class EmailService {
	...
	public function sendActivationEmail($account, $site_url) {
		...
		$this->_mailer->Body = \MailLoader::loadHtml('activation_email', array(
			'account' => $account,
			'site_url' => $site_url
		));
		
		$this->_mailer->AltBody = \MailLoader::loadText('activation_email', array(
			'account' => $account,
			'site_url' => $site_url
		));
	...
	public function sendPostComment($comment, $post, $poster, $commenter) {
		...
		$this->_mailer->Body = \MailLoader::loadHtml('post_comment', array(
			'comment' => $comment,
			'post' => $post,
			'poster' => $poster,
			'commenter' => $commenter
		...
		$this->_mailer->AltBody = \MailLoader::loadText('post_comment', array(
			'comment' => $comment,
			'post' => $post,
			'poster' => $poster,
			'commenter' => $commenter
