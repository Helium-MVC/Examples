                ...
                if (selectedElement.tagName !== undefined) {
                    if (selectedElement.tagName.toLowerCase() === 'div' &&
                        ...
                        if (/<span>(<span>)+/i.test(__h)) {
                            __h = __h.replace(/<span>(<span>)+/i, '<span>');
                        }
                        // remove stacked up </span>'s
                        if (/<\/span>(<\/span>)+/i.test(__h)) {
                            __h = __h.replace(/<\/span>(<\/span>)+/i, '<\/span>');
                            ...
                            if(text.match(/class=["']*Mso(Normal|List)/i) || text.match(/content=["']*Word.Document/i) || text.match(/content=["']*OneNote.File/i)){
                                ...
                                if(text.match(/<[^>]*?(ta-bind)[^>]*?>/)){
                                    ...
                                    if(text.match(/<[^>]*?(text-angular)[^>]*?>/)){
                                        ...
                                        for(var _b = 0; _b < window.binds.length; _b++){
                                            var _target = window.binds[_b][0].parentNode.parentNode;
                                            for(var _c = 0; _c < window.binds[_b][0].childNodes.length; _c++){
                                                _target.parentNode.insertBefore(window.binds[_b][0].childNodes[_c], _target);
                                            }
